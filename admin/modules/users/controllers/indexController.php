...
function registerAction() {
    ...
    if(isset($_POST['btn'])) {
        ...
        if(empty($_POST['username'])){
            $error['username'] = "Vui long nhap truong nay";
        }else {
            if(is_username($_POST['username'])) {
                $username = $_POST['username'];
            }else {
                $error['username'] = "Dinh dang username chua dung";
            }
        }
        ...
        if(empty($error)) {
            if(user_exists($email)){
                ...
                send_mail($email, $username, "Kich hoat tai khoan tai website cua Soi Xam", $content);
    ...
    if(isset($_POST['btn'])) {
        ...
        if(empty($error)) {
            if(check_login($email, $password)){
                ...
                $error['account'] = "Có lỗi đăng nhập, vui lòng kiểm tra các trường hợp sau
                (1)Email hoặc mật khẩu không chính xác
                (2)Tài khoản của bạn chưa được xác thực, 
                vui lòng kiểm tra email đã đăng ký tài khoản để tiến hành xác thực
                (3)Bạn không có quyền truy cập hệ thống Admin.";
            }
        }
    }
...
function infoAccountAction() {
    global $error, $name, $email, $phone, $address, $gender, $status;
...
function changePassAction() {
    global $error, $email, $password, $password_1, $password_2, $status;
    $email = $_SESSION['user_login'];
